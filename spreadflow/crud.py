...
from models import User, Subscription, ActivityLog, Invite, Plan, Payment, SupportTicket
...
# === НОВОЕ: ОБНОВЛЕНИЕ РОЛИ ===
def update_user_role(db: Session, user_id: str, new_role: str):
    """Обновляет роль пользователя (user, moderator, admin)"""
    try:
...
# --- СИСТЕМА ПОДДЕРЖКИ (НОВОЕ) ---

def create_support_ticket(db: Session, user_id, subject, message, contact=None):
    """Создает новый тикет"""
    try:
        ...
        db.add(new_ticket)
        db.commit()
        return True, "Обращение успешно отправлено"
    except Exception as e:
        db.rollback()
        log.error(f"Ticket create error: {e}")
        return False, str(e)

def get_all_tickets(db: Session):
    """Для админа: все тикеты, новые сверху"""
    return db.query(SupportTicket).join(User).order_by(SupportTicket.created_at.desc()).all()

def update_ticket_status(db: Session, ticket_id, new_status):
    """Для админа: сменить статус (OPEN/CLOSED)"""
    try:
        db.query(SupportTicket).filter(SupportTicket.id == ticket_id).update({"status": new_status})
        db.commit()
        return True
    except:
        db.rollback()
...
def delete_ticket_db(db: Session, ticket_id):
    """Удалить тикет"""
    try:
