...
def get_price_worker(args):
    ...
    try:
        ...
        price = ticker['last']
        # Берем quoteVolume (объем в долларах/USDT), если нет - пробуем baseVolume * price
        vol = ticker.get('quoteVolume') 
        if vol is None:
             vol = ticker.get('baseVolume', 0) * price

        if price and price > 0:
            # Добавляем 'vol' в результат
...
def calculate_logic(raw_data):
    ...
    min_vol = app_state.get("min_volume", 0) # Берем настройку из state
    ...
    for sym, prices in grouped.items():
        if len(prices) > 1:
            ...
            # ФИЛЬТР ПО ОБЪЕМУ:
            # Проверяем объем и на бирже покупки, и на бирже продажи
            # Если хотя бы на одной объем меньше заданного - пропускаем
            if min_p['vol'] < min_vol or max_p['vol'] < min_vol:
                continue
