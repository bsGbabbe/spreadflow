        ...
        with ui.column().classes('w-full p-8 max-w-7xl mx-auto gap-8'):
            ...
            with ui.tab_panels(tabs, value=t_users).classes('w-full bg-white rounded-b-xl p-6 shadow-sm'):
                    ...
                    def open_user_edit(user_id, username):
                        ...
                        with user_edit_dialog, ui.card().classes('w-96'):
                            ...
                            def save_user_settings():
                                ...
                                if ov_spread.value: overrides['max_spread'] = int(ov_spread.value)
                                if ov_speed.value: overrides['refresh_rate'] = int(ov_speed.value)
                ...
                with ui.tab_panel(t_invites):
                    # (Код инвайтов тот же, что и был раньше)
                    ...
                    with ui.row().classes('gap-2 items-end mb-4'):
                        ...
                        il = ui.number('Лимит', value=1)
                        def gen_inv():
                            db=next(get_db())
                            create_invite_db(db, ic.value, ip.value, int(il.value))
                            db.close()
                            ui.notify('Создано')
                        ui.button('Generate', on_click=gen_inv)
