...
def create_ui():
    ...
    with ui.column().classes('w-full p-8 gap-0'):
        ...
        def ui_tick():
            ...
            # Если это Free тариф, то limits['refresh_rate'] = 30
            # Мы проверяем: Текущее время - Время последнего обновления UI > 30 сек?
            
            time_since_last_render = time.time() - app_state.get('ui_updated_ts', 0)
            
            if time_since_last_render >= limits['refresh_rate']:
                # Но обновляем только если в backend реально есть новые данные
